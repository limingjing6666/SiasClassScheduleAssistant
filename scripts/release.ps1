...
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
    Write-Host "[错误] 未安装 GitHub CLI。请运行: winget install GitHub.cli" -ForegroundColor Red
    exit 1
}
...
# ═══════════════════════════════════════════════
if (-not $ApkPath) {
    Write-Host "[查找] 正在查找最新的 APK 文件..." -ForegroundColor Cyan

    # HBuilderX 常见的 APK 输出路径
    $searchPaths = @(
        # HBuilderX 输出目录（dist 和 unpackage 两种结构）
    ...
    if (-not $latestApk) {
        Write-Host "[错误] 未找到 APK 文件！请先在 HBuilderX 中完成打包，或使用 -ApkPath 参数指定路径。" -ForegroundColor Red
        exit 1
    }

    $ApkPath = $latestApk.FullName
    Write-Host "[OK] 找到 APK: $ApkPath" -ForegroundColor Green
    ...
    if ($latestApk.LastWriteTime -lt (Get-Date).AddHours(-1)) {
        Write-Host "[警告] 该 APK 是 $([math]::Round(((Get-Date) - $latestApk.LastWriteTime).TotalHours, 1)) 小时前生成的，确定使用？(Y/n)" -ForegroundColor Yellow
        $confirm = Read-Host
...
if (-not $Version) {
    ...
    if ($latestTag -match "v?(\d+)\.(\d+)\.(\d+)") {
        ...
        Write-Host "[版本] 上一版本: $latestTag -> 新版本: v$Version" -ForegroundColor Cyan
    } else {
        ...
        if (Test-Path $manifestPath) {
            ...
            Write-Host "[版本] 从 manifest.json 读取版本: v$Version" -ForegroundColor Cyan
        } else {
            $Version = "1.0.0"
        }
    }
...
if ($ApkPath -ne $CleanApkPath) {
    ...
    Write-Host "[重命名] 已复制 APK 为: $CleanApkName" -ForegroundColor Green
...
Write-Host ""
Write-Host "[发布] 正在创建 Release $Tag 并上传 APK..." -ForegroundColor Cyan
...
if ($releaseExists) {
    Write-Host "[警告] Release $Tag 已存在，是否覆盖上传？(Y/n)" -ForegroundColor Yellow
    $confirm = Read-Host
    if ($confirm -eq 'n' -or $confirm -eq 'N') {
        exit 0
    }
    ...
    Write-Host "[OK] 已覆盖上传 APK 到 Release $Tag" -ForegroundColor Green
} else {
    # 创建新 Release
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Release $Tag 创建成功！APK 已上传。" -ForegroundColor Green
        Write-Host "[部署] GitHub Actions 将自动部署 APK 到服务器..." -ForegroundColor Cyan
    } else {
